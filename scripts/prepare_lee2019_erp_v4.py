#!/usr/bin/env python
"""
Prepare Lee2019_ERP with 128 pipeline variants.
Auto-downloads via MOABB. Binary P300 (target vs non-target), 62 channels.
Output: data/processed_v4/lee2019_erp/S{id}/all_pipelines.npy + labels.npy
"""
...
def main(args):
    out_dir = Path(args.output_dir) / "lee2019_erp"
    out_dir.mkdir(parents=True, exist_ok=True)

    dataset = Lee2019_ERP()
    paradigm = P300(fmin=0.1, fmax=45, resample=256)
    ...
    for subj in subjects:
        ...
        # Subsample to max 1000 trials (62ch × 4140 trials × 128 pipelines = 31GB/subject)
        if len(X) > 1000:
            idx = np.random.choice(len(X), 1000, replace=False)
            idx.sort()
            X = X[idx]
            labels = labels[idx]
            logger.info(f"  Subsampled to {len(X)} trials")
